...
public class Message implements Serializable{
	...
	private static final int POSITION_ID_GAME = 3;
	private static final int POSITION_TYPE = 4;
	private static final int POSITION_PARAMS = 5;
	...
	public static String createMessage(Ipv4 ip, Integer port, Integer id_game, String type, String params){
		if(!ParamCheck.notNull(new Object[]{ ip, port, id_game, type}))
			return null;
		...
		String idgamestring = Integer.toString(id_game);
		
		return concat(SEPA, new String[]{ipstring, portstring, idgamestring, type, params});
	}
	...
	/**
	 * Return the sender's game id target.
	 * 
	 * @param message
	 *            a Message format string.
	 * @return the sender's game id target
	 */
	public static Integer getIdGame(String message) {
		if (message == null)
			return null;
		...
		if(SEPA == null)return null;
		try {
			message = message.split(SEPA)[POSITION_ID_GAME];
		} catch (ArrayIndexOutOfBoundsException a) {
			return null;
		}
	...
	private static int getTailleTypeMessage(String type){
		...
		if(type.equals(MessageCG.CG_TYPE_GAME_NAME))
			return MessageCG.CG_TYPE_GAME_NUM_PARAMS;
		if(type.equals(MessageCG.CG_NEW_GAME_NAME))
			return MessageCG.CG_NEW_GAME_NUM_PARAMS;
		if(type.equals(MessageCG.CG_MIN_PLAYER_NAME))
			return MessageCG.CG_MIN_PLAYER_NUM_PARAMS;
		if(type.equals(MessageCG.CG_MAX_PLAYER_NAME))
			return MessageCG.CG_MAX_PLAYER_NUM_PARAMS;
		if(type.equals(MessageCG.CG_START_GAME_NAME))
			return MessageCG.CG_START_GAME_NUM_PARAMS;
		...
		if(type.equals(MessageCS.CS_IDENTIFICATION_NAME))
			return MessageCS.CS_IDENTIFICATION_NUM_PARAMS;
		if(type.equals(MessageCS.CS_INSCRIPTION_NAME))
			return MessageCS.CS_INSCRIPTION_NUM_PARAMS;
		...
		if(type.equals(MessageSS.SS_IDENTIFICATION_NAME))
			return MessageSS.SS_IDENTIFICATION_NUM_PARAMS;
		if(type.equals(MessageSS.SS_INSCRIPTION_NAME))
			return MessageSS.SS_INSCRIPTION_NUM_PARAMS;
		if(type.equals(MessageSS.SS_NEW_GAME_NAME))
			return MessageSS.SS_NEW_GAME_NUM_PARAMS;
	...
	public static boolean isNotTypeListeParam(String message){
		...
		if(type.equals(MessageCG.CG_CHAT_NAME)
				...
				|| type.equals(MessageCG.CG_TYPE_GAME_NAME)
				|| type.equals(MessageCG.CG_NEW_GAME_NAME)
				|| type.equals(MessageCG.CG_START_GAME_NAME)
				|| type.equals(MessageCG.CG_MAX_PLAYER_NAME)
				|| type.equals(MessageCG.CG_MIN_PLAYER_NAME)
				...
				|| type.equals(MessageSS.SS_IDENTIFICATION_NAME)
				|| type.equals(MessageSS.SS_INSCRIPTION_NAME)
				|| type.equals(MessageSS.SS_NEW_GAME_NAME)
				|| type.equals(MessageCS.CS_INSCRIPTION_NAME)
				|| type.equals(MessageCS.CS_IDENTIFICATION_NAME))
			return true;
	...
	public static boolean isTypeListeParam(String message){
		...
		if(type.equals(MessageSR.SR_AVATARS_NAME)
				|| type.equals(MessageSR.SR_STATES_NAME)
