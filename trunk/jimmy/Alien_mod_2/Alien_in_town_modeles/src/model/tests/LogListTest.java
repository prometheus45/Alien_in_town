...
public class LogListTest extends TestCase {
	...
	public void testLogListArrayListOfLog() {
		...
		String message = MessageSP.disconnect(ip, 1, 0);
		try {
	...
	public void testAddString() {
		...
		Ipv4 ip = new Ipv4("0.0.0.0");
		String message = Message.createMessage(ip, 1, 0, MessageSP.SP_DISCONNECT_NAME, null);
	...
	public void testRemove() {
		...
		ArrayList<Log> liste = new ArrayList<Log>();
		String message = MessageSP.disconnect(ip, 1, 0);
		String message2 = MessageSP.disconnect(ip, 2, 0);
		try {
			liste.add(new Log(message));
